...
return (
    <section className={styles.game}>
      <div className={styles.game__info}>
        {!isCircleVisible && <p className={styles.game__ratio}>
...
{    !  (!isFroget && isCircleVisible && !isDisplaying)  && <div className={`${styles.game__froget} `} >
          <div className={`${styles.game__frog}  ${(isStartedRecently && !isFinishedRecently) ? styles.game__flyFromButton : ""} ${(isFinishedRecently) ? styles.game__flyTopRight : ""}`} style={!isFroget ? { display: 'none' } : null}/>
        </div>}
  ...
  (!isFroget && isCircleVisible && !isDisplaying) && (
    <div className={styles['game__circle-container']}>
      ...
      <div className={styles['game__modale-container']}>
         ...
         {((isDisplaying || (currentBet.current && game && currentBet.current.round_id == game.id &&( resultsOfGame.current.isSuccess)) ) ||   (isError.current))&& (<WinLoseBoard errorMsg={errorMsg.current!=1 ? errorMsg.current : 1} isSuccess={isError.current? !(isError.current) :resultsOfGame.current.isSuccess} rate={isError.current ? '':resultsOfGame.current.amount.toFixed(2)} />)}
      </div>
      <div className={cn(
          styles.game__clouds,
